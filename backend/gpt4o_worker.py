...
class Worker:
    ...
    def generate_summary(self,extraction, prompt):
        summary_prompt = (
            f"Based on the following extraction and the question, provide a detailed summary if the question is available in the PDF, otherwise indicate unavailability and you should not provide any headings and return all the context in markdown:\n\n"
            f"Extraction:\n{extraction}\n\n"
            f"Question:\n{prompt}\n\n"
    ...
    def generate_reference_clause(self,extraction):
        matching_prompt = (
            f"Identify and return the most relevant heading and name it as Heading. The heading should be selected based on the presence of a line that starts with either a number, a Roman numeral, or a newline character and ends with a colon, or starts and ends with two newline characters and do not include the word heading in the result again.\n\n"
            f"Text:\n{extraction}\n\n"
            f"Relevant Heading:"
        )
        response = self.llm_hub.generate([matching_prompt])
